...
//    }
    ...
    private void putInteractingMobBlock(InteractingMobBlock interactingMobBlock, GamePacketBuilder blockBuilder) {
        blockBuilder.put(DataType.SHORT, interactingMobBlock.getIndex());
    }
...
//		playerProperties.put(DataType.SHORT, animationSet.getRunning()); // run
    ...
    private void putAnimationBlock(AnimationBlock animationBlock, GamePacketBuilder blockBuilder) {
        ...
        blockBuilder.put(DataType.SHORT, DataTransformation.ADD, anim.getId());
        blockBuilder.put(DataType.BYTE, anim.getDelay());
    }
    ...
    private void putTurnToPositionBlock(TurnToPositionBlock turnToPositionBlock, GamePacketBuilder blockBuilder) {
        ...
        if (loc == null) {
            blockBuilder.put(DataType.SHORT, DataOrder.LITTLE, DataTransformation.NONE, 0);
            blockBuilder.put(DataType.SHORT, DataOrder.LITTLE, DataTransformation.ADD, 0);
        } else {
            blockBuilder.put(DataType.SHORT, DataOrder.LITTLE, DataTransformation.NONE, loc.getX() * 2 + 1);
            blockBuilder.put(DataType.SHORT, DataOrder.LITTLE, DataTransformation.ADD, loc.getY() * 2 + 1);
        }
    }
}
